class Controller:
    def __init__(self, motion: float, wheel: float):
        self.mouse_sensitivity = (motion, wheel)
    ...
    def mouse_motion(self, screen_position: tuple[int, int], time: int) -> None:
        if self.mouse_selection:
            ...
            self.mouse_movement[0] += pixel_y * time * self.mouse_sensitivity[0]
            self.mouse_movement[1] += pixel_x * time * self.mouse_sensitivity[0]

            self.mouse_position = screen_position

    def mouse_wheel(self, scale_z: int, time: int) -> None:
        self.mouse_movement[2] += scale_z * time * self.mouse_sensitivity[1]
